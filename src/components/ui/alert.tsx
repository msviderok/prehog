...
function Alert(props: ComponentProps<'div'> & VariantProps<typeof alertVariants>) {
  ...
  return (
    <div data-slot="alert" role="alert" class={cn(alertVariants({ variant: local.variant }), local.class)} {...rest} />
  )
}
...
function AlertAction(props: ComponentProps<'div'>) {
  const [local, rest] = splitProps(props, ['class'])
  return <div data-slot="alert-action" class={cn('absolute top-1.5 right-2', local.class)} {...rest} />
}
